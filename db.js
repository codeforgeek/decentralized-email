    ...
    fs.access(filePath, fs.F_OK, async (err) => {
      if (err) {
        ...
        try {
          ...
          userContactsDb = await orbitdb.create(
            "email.user.contacts",
            "docstore",
            {
              accessController: {
          ...
          console.log("[Debug] An error occured", error);
        }
      } else {
...
async function getUserContacts(data) {
  try {
    let userContactData = userContactsDb.query(
      (doc) => doc.userId === data.userId
    );
    return {
      error: false,
...
async function findIfContactExists(userId, contactId) {
  try {
    let userContactData = userContactsDb.query(
      (doc) => doc.userId === userId && doc.contactId === contactId
    );
    return {
      error: false,
      data: userContactData,
...
async function getUserContactsRequest(data) {
  try {
    ...
    let userContactRequestData = userContactsDb.query(
      (doc) => doc.contactEmail === data.email && doc.status === 0
    );
...
async function addUserContact(data) {
  try {
    ...
    let checkIfExists = await findIfContactExists(
      data.userId,
      fetchContactData[0]["_id"]
    );
...
async function sendEmail(data) {
  ...
  try {
    ...
    let signatureData = signMessage(email, {
      privateKey: senderData[0].privateKey,
      publicKey: reciepentData[0].publicKey,
    });
    let emailData = {
      _id: id,
      from: data.from,
...
async function readEmail(data) {
  try {
    ...
    let userData = await getUserByEmail(data.email);
    let senderData = await getUserByEmail(
      data.source === "inbox" ? emailData[0].from : emailData[0].to
    );
